# Три друга взяли вещи в поход. Сформируйте словарь, где ключ — имя друга, а значение — кортеж вещей.
# Ответьте на вопросы:
# 1) Какие вещи взяли все три друга
# 2) Какие вещи уникальны, есть только у одного друга и имя этого друга
# 3) Какие вещи есть у всех друзей кроме одного и имя того, у кого данная вещь отсутствует
# Для решения используйте операции с множествами. Код должен расширяться на любое большее количество друзей.d

data = {
    'Петя': ('Палатка', 'Спальник', 'Фонарик', 'Газовая горелка'),
    'Иван': ("Палатка", "Спальник", "Тент", "Перчатки"),
    "Кристина": ("Палатка", "Спальник", "Фонарик", "Кружка")
}


print('кек')

# К сожалению не успеваю сделать эту задачу вовремя(( Обязательно доделаю ее в течении нескольких дней
